...
def select_from_unvisited(unvisited: dict[MazeCell, int]) -> MazeCell:
    """
    ...
    for cell in unvisited:
        ...
        if not cell.distance_to_entrance == -1:
            if not current_cell or \
             current_cell.distance_to_entrance > cell.distance_to_entrance:
                current_cell = cell
...
def set_neighbors_distance(
        maze: Maze,
        unvisited: list[MazeCell],
        maze_width: int,
        current_cell: MazeCell
) -> None:
    """
    Sets current cell's neighors distance to entrance
    """
    ...
    if not current_cell.walls & 1:
        ...
        if north_neighbor.distance_to_entrance == -1 or \
         new_distance < north_neighbor.distance_to_entrance:
    ...
    if not current_cell.walls & 2:
        ...
        if east_neighbor.distance_to_entrance == -1 or \
             new_distance < east_neighbor.distance_to_entrance:
    ...
    if not current_cell.walls & 4:
        ...
        if south_neighbor.distance_to_entrance == -1 or \
             new_distance < south_neighbor.distance_to_entrance:
    ...
    if not current_cell.walls & 8:
        ...
        if west_neighbor.distance_to_entrance == -1 or \
             new_distance < west_neighbor.distance_to_entrance:
...
def _closest_neighbor(
    ...
    if not current_cell.walls & 1:
        north_neighbor = maze[current_cell.INDEX - maze_width]
        all_neighbors.append((north_neighbor, 'N'))

    # Check if East wall is present
    if not current_cell.walls & 2:
        east_neighbor = maze[current_cell.INDEX + 1]
        all_neighbors.append((east_neighbor, 'E'))

    # Check if South wall is present
    if not current_cell.walls & 4:
        south_neighbor = maze[current_cell.INDEX + maze_width]
        all_neighbors.append((south_neighbor, 'S'))

    # Check if West wall is present
    if not current_cell.walls & 8:
        west_neighbor = maze[current_cell.INDEX - 1]
        all_neighbors.append((west_neighbor, 'W'))

    closest_neighbor = None

    for neighbor in all_neighbors:
        if not closest_neighbor or neighbor[0].distance_to_entrance < \
...
def path_to_entrance(
        maze: Maze, exit_cell: MazeCell, maze_width: int
) -> list[str]:
    """
